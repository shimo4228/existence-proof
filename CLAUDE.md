...
AI-powered humans（仮称 — layer 名は suspend 中、doctrine §12）のエンパワーメント戦略 doctrine v2。機構は **oracle debiasing**（[ADR-0002](docs/adr/0002-gatekeeper-inversion-strategic-reframe.md)）: 機械検証可能な存在証明を LLM が feasibility 質問を評価する面に置く。**research line ではない** — DOI 未取得・GitHub 未公開の working repo であり、「sibling」表記は使わない（sibling は research line レベル予約語 — authorship-strategy CLAUDE.md の terminology 規約）。昇格条件は [docs/doctrine.md §9](docs/doctrine.md)。
...
- 命名は二 track（doctrine §12）: layer 名 = suspend（再オープンしない）/ format 名 = 暫定「存在証明フォーマット」swap-ready（初出フル名 + 暫定 note、以降 EPF。bare 略語を primary token にしない）
