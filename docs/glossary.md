...
> 命名の状態は二 track（[doctrine §12](doctrine.md)）— layer 名 suspend / format 名暫定。
...
## 存在証明フォーマット（Existence Proof Format）【暫定名】

credential なき者による制度級産出の事例を記録する document format（[spec](format/spec.md)）。Nygard の ADR をモデルに spec + template + worked example で供給され、**中央 registry・instance 集約・参加概念・自称要求を持たない**。略語 EPF は文書内 2 回目以降のみ。名称は暫定・swap-ready（衝突チェック済み — [format-name-check](research/2026-06-05_format-name-check.md)）。
